...
import TimeField from "../components/TimeField"; // 24h time picker
...
function MobileServicesList({
  ...
  if (!items?.length) {
    return (
      <div className="sm:hidden p-3 rounded-2xl border text-gray-500">
        Nėra įrašų.
      </div>
    );
  }
  return (
    <div className="sm:hidden space-y-2">
      {items.map((svc) => {
        const ref =
          (menuBtnRef.current[svc.id] =
            menuBtnRef.current[svc.id] || React.createRef());
        const isDeleting = deletingId === svc.id;
        return (
          <div
            key={svc.id}
            className={`relative rounded-2xl border p-3 ${isDeleting ? "opacity-50" : ""}`}
          >
            <div className="flex items-start gap-2">
              ...
              <DotMenu
                ...
                items={[
                  { label: "Koreguoti", onClick: () => onEdit(svc) },
                  {
...
export default function SettingsServices({ workspace }) {
  ...
  function validateCreate() {
    ...
    if (!validHex(categoryColor)) return { ok: false, reason: "Neteisinga kategorijos spalvos reikšmė." };
  ...
  async function createServices() {
    ...
    const category = norm(catName);
    const subs = rows
      .map((r) => ({ name: norm(r.name), price: norm(r.price), color: norm(r.color) }))
  ...
  function validateEdit() {
    if (!norm(editForm.category)) return { ok: false, reason: "Kategorija negali būti tuščia." };
    if (!validHex(norm(editForm.color))) return { ok: false, reason: "Neteisinga spalvos reikšmė." };
  ...
  return (
    <div className="bg-white rounded-2xl shadow p-4 sm:p-5 space-y-4">
      ...
      <div className="border rounded-2xl p-3 sm:p-4">
        <div className="flex items-center justify-between mb-3 gap-2">
          <div className="text-base sm:text-lg font-semibold">
            Darbo laikas
            <Tooltip content="Naudojama laisvų tarpų skaičiavimui dienos grafike.">
              <HelpDot />
            </Tooltip>
          </div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 items-end">
          ...
          <div>
            ...
            <TimeField value={workEnd} onChange={setWorkEnd} step={60} />
          </div>
      ...
      <MobileServicesList
        items={filtered}
        onEdit={openEdit}
        onRemove={confirmAndRemoveService}
